...
export default function AttendanceManagementPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [attRes, divRes, empRes, batchRes] = await Promise.all([
        getAttendanceManagement({
          month: selectedMonth,
          year: selectedYear,
  ...
  const pendingLateReasonCount = records.filter((r) => r.late_minutes > 0 && !r.late_reason).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Impor rekap mesin fingerprint (Solution, Fingerspot, ZKTeco) dan monitoring kehadiran seluruh karyawan.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-sm">
          <div className="flex items-center justify-between text-slate-400 mb-2">
            <span className="text-xs font-medium uppercase tracking-wider font-mono">Alasan Terlambat Pending</span>
            <div className="w-8 h-8 rounded-lg bg-rose-500/10 flex items-center justify-center text-rose-400">
              <AlertTriangle className="w-4 h-4" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-bold text-rose-400 font-mono">{pendingLateReasonCount}</span>
            <span className="text-xs text-slate-500">belum diisi karyawan</span>
          </div>
        </div>
      </div>
                ...
                <tr>
                  <td colSpan={8} className="px-5 py-12 text-center text-slate-500">
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Gunakan tombol "Import Mesin Fingerprint" untuk mengunggah berkas Excel/CSV kehadiran.
                    </p>
                  </td>
                </tr>
              ) : (
