...
interface RequestTypesClientProps {
...
export function RequestTypesClient({
  initialTypes,
  initialError = null,
}: RequestTypesClientProps) {
  const [types, setTypes] = useState<RequestType[]>(initialTypes);
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    ...
    if (!res.success) {
      setError(res.error || 'Gagal mengubah status jenis pengajuan');
    } else {
      loadData();
    }
  };
