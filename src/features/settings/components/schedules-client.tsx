...
import { Plus, Edit2, Trash2, Clock, AlertCircle, Coffee } from 'lucide-react';
import {
  getScheduleGroups,
  saveScheduleGroup,
  deleteScheduleGroup,
...
interface SchedulesClientProps {
  initialGroups: (WorkScheduleGroup & { days?: WorkScheduleDay[] })[];
...
export function SchedulesClient({
  ...
  const [groups, setGroups] = useState<(WorkScheduleGroup & { days?: WorkScheduleDay[] })[]>(initialGroups);
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    if (!res.success) {
      setError(res.error || 'Gagal menghapus kelompok jadwal');
    } else {
      reloadData();
    }
  };
